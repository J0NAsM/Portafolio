...
function cvPage() {
  ...
  return `<main id="main" class="page cv-page"><div class="cv-toolbar no-print">${sectionTitle('CV ATS', 'Un CV simple, legible y preparado para completar.', 'Esta versión evita columnas, tablas, íconos y gráficos. Completa los campos entre corchetes con información verificable antes de imprimir o guardar como PDF.')}<div class="cv-actions"><button class="button" type="button" data-print-cv>Imprimir CV <span>→</span></button></div><p class="ats-note"><strong>Uso recomendado:</strong> adapta las competencias y viñetas a una oferta concreta, usando únicamente palabras clave y resultados que puedas demostrar.</p></div><article class="cv-document" aria-label="Currículum ATS imprimible"><header><h1>${profile.name}</h1><p>${field('city', '[Ciudad, País]', 'Ciudad y país')} | ${field('phone', '[Teléfono]', 'Teléfono')} | ${field('email', '[Email profesional]', 'Email profesional')}</p><p>LinkedIn: ${field('linkedin', '[linkedin.com/in/usuario]', 'LinkedIn')} | GitHub: ${field('github', '[github.com/usuario]', 'GitHub')}</p></header><section><h2>PERFIL PROFESIONAL</h2><p>Profesional en formación en Análisis de Sistemas Informáticos, orientado al desarrollo de software y al análisis de procesos empresariales. Interés y experiencia práctica en sistemas con reglas de negocio, datos, permisos, flujos operativos y digitalización institucional.</p></section><section><h2>COMPETENCIAS CLAVE</h2><p><strong>Desarrollo:</strong> Java, Python, JavaScript, HTML, CSS, React, APIs, GeneXus.</p><p><strong>Datos:</strong> SQL, PostgreSQL, MySQL, SQLite, modelado y consulta de datos.</p><p><strong>Sistemas:</strong> análisis de sistemas, reglas de negocio, roles y permisos, flujos de autorización, auditoría y procesos transaccionales.</p><p><strong>Herramientas:</strong> Git, GitHub, DBeaver, GeneXus, Obsidian, documentación técnica e IA aplicada al desarrollo.</p></section><section><h2>EXPERIENCIA LABORAL</h2><p class="cv-instruction no-print">Completa solo experiencias, roles, periodos y resultados que puedas verificar. Mantén el orden cronológico inverso.</p><div class="cv-entry"><p><strong>Softshop</strong> | ${field('softshop-place', '[Ciudad, País]', 'Ciudad y país de Softshop')}</p><p><strong>${field('softshop-role', '[Cargo formal] · GeneXus', 'Cargo formal en Softshop')}</strong> | ${field('softshop-date', '[Mes AAAA] – [Actualidad o Mes AAAA]', 'Periodo en Softshop')}</p><p>• Experiencia profesional con GeneXus en el desarrollo de soluciones de software.</p><p class="cv-field" contenteditable="true" role="textbox" aria-label="Responsabilidad verificable en Softshop" data-cv-field="softshop-bullet">• [Agrega una responsabilidad, módulo o resultado verificable.]</p></div><div class="cv-entry"><p><strong>${field('company-2', '[Organización anterior]', 'Organización anterior')}</strong> | ${field('place-2', '[Ciudad, País]', 'Ciudad y país de organización anterior')}</p><p><strong>${field('role-2', '[Cargo estándar de mercado]', 'Cargo anterior')}</strong> | ${field('date-2', '[Mes AAAA] – [Mes AAAA]', 'Periodo anterior')}</p><p class="cv-field" contenteditable="true" role="textbox" aria-label="Responsabilidad verificable" data-cv-field="bullet-3">• [Acción + contexto + herramienta + resultado verificable.]</p></div></section><section><h2>EDUCACIÓN</h2><p><strong>Análisis de Sistemas Informáticos</strong> | ${field('institution', '[Institución]', 'Institución educativa')} | ${field('education-date', '[Año de inicio] – [Año de finalización o Actualidad]', 'Periodo educativo')}</p></section><section><h2>IDIOMAS Y CERTIFICACIONES</h2><p><strong>Idiomas:</strong> ${field('languages', '[Idioma] ([Nivel CEFR: A1–C2])', 'Idiomas y niveles')}</p><p><strong>Certificaciones:</strong> ${field('certifications', '[Agregar solo certificaciones verificables]', 'Certificaciones')}</p></section></article></main>`;
}
