...
const backupDirectory = root => path.join(root, 'data', 'blog-backups');
...
export async function writeBlogPosts(root, posts) {
  ...
  try {
    const previous = await fs.readFile(contentFile(root), 'utf8');
    const directory = backupDirectory(root);
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    await fs.mkdir(directory, { recursive: true });
    await fs.writeFile(path.join(directory, `blog-posts-${timestamp}.json`), previous, 'utf8');
    const backups = (await fs.readdir(directory)).filter(name => name.endsWith('.json')).sort().reverse();
    await Promise.all(backups.slice(backupLimit).map(name => fs.rm(path.join(directory, name), { force: true })));
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  await Promise.all([
    fs.mkdir(path.dirname(contentFile(root)), { recursive: true }),
    fs.mkdir(path.dirname(browserFile(root)), { recursive: true })
  ]);
